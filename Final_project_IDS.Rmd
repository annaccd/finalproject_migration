...
data %>% 
 filter(GEO=="European Union - 27 countries (from 2020)" | GEO=="European Union - 28 countries (2013-2020)") %>% filter(ISCO08=="Total")%>% group_by(DECISION,TIME) %>% ungroup()%>%
  plot_ly(x=~TIME, y=~Value, group=~DECISION,
...
B.OCCUPATIONS
```{r echo=FALSE}
data %>%filter(DECISION=="Granted")%>% filter(GEO=="European Union - 27 countries (from 2020)" | GEO=="European Union - 28 countries (2013-2020)")%>%filter(ISCO08!="Total" & ISCO08!="Unknown")%>%filter(TIME!="2020")%>%
  group_by(ISCO08,TIME)%>%ungroup()%>%
  plot_ly(x=~TIME, y=~Value, group=~ISCO08,
